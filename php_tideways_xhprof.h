...
typedef struct xhprof_frame_tt xhprof_frame_tt;
...
struct xhprof_frame_tt {
    struct xhprof_frame_tt   *previous_frame;        /* ptr to prev entry being profiled */
    zend_string         *function_name;
    zend_string         *class_name;
    uint64              wt_start;           /* start value for wall clock timer */
    uint64              wt_end;
...
ZEND_BEGIN_MODULE_GLOBALS(tideways_xhprof)
    int enabled;
    long int n_frame;
    xhprof_frame_tt *my_frame;
    xhprof_frame_tt *frame_list[10000];
